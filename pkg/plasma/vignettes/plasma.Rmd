...
```{r fig01, out.width = "100%", fig.cap = .tag(1, "Workflow schematic for plasma algorithm with three omics data sets."), echo = FALSE}
...
image(1:24, 1:7, t(summer$Mean), xlab = "Component", ylab = "Dataset", main = "Mean",
      zlim = c(-mx, mx), col = redgreen(64))
image(1:24, 1:7, t(summer$SD), xlab = "Component", ylab = "Dataset", main = "SD",
      zlim = c(0, sx), col = bluescale(64))
image(1:24, 1:7, t(summer$Median), xlab = "Component", ylab = "Dataset", main = "Median",
      zlim =c(-dx, dx), col = redgreen(64))
image(1:24, 1:7, t(summer$MAD), xlab = "Component", ylab = "Dataset", main = "Mad",
      zlim = c(0, ax),col = bluescale(64))
par(opar)
rm(opar)
```
...
```{r}
foo <- lapply(names(pl@compModels), function(N) getAllWeights(pl, N)@contrib)
names(foo) <- names(pl@compModels)
contra <- do.call(rbind, foo)
featsize <- sapply(foo, nrow)
datasrc <- factor(rep(names(foo), times = featsize))
```


### Raw Weights
In **Figure 10**, we create beanplots showing the distributions of weights arising from each
...
```{r fig10, fig.cap = .tag(10, "Distributions of weights by data set and component."), fig.width = 12, fig.height = 18}
library(beanplot)
library(Polychrome)
data(palette36)
...
for (i in which(colnames(contra) %in% mainterms)) {
  beanplot(contra[, i] ~ datasrc, what = c(1,1,1,0), col = colist,
           main = paste("Component ", i, " (", colnames(contra)[i], ")", sep = ""))
}
par(opar)
rm(opar)
```

**Figure 11** lumps the weights from all data sets together to view the distributions by component.
...
```{r fig11, fig.cap = .tag(11, "Histogram of weights by component."), fig.width = 12, fig.height = 18}
opar <- par(mfrow = c(3, 3), mai = c(0.4,0.4, 0.5, 0.2))
for (i in which(colnames(contra) %in% mainterms)) {
...
In **Figure 12**, we have standardized the weights in each data set and component, and recreate the
corresponding beanplot distributions. The standardization has moved the outliers away from the three
previous data sets (clinical binary, MAF, and RPPA) primarily to the miRs, though with more dispersed
contributions as well.
```{r fig12, fig.cap = .tag(12, "Distributions of standardized weights by data set and component."), fig.width = 12, fig.height = 18}
M <- apply(summer$Mean, 2, function(X) rep(X, times = CW@featureSize))
S <- apply(summer$SD, 2, function(X) rep(X, times = CW@featureSize))
brute <- (contra - M)/S
opar <- par(mfrow = c(3,3), mai = c(0.2,0.2, 0.5, 0.2))
for (i in which(colnames(contra) %in% mainterms)) {
...
M <- apply(summer$Median, 2, function(X) rep(X, times = CW@featureSize))
S <- apply(summer$MAD, 2, function(X) rep(X, times = CW@featureSize))
robust <- (contra - M)/S
opar <- par(mfrow = c(6, 4), mai = c(0.2,0.2, 0.5, 0.2))
for (i in 1:24) {
  beanplot(robust[, i] ~ datasrc, what = c(1,1,1,0), col = colist,
...
In **Figure 24**, we pool all the weights 9across all data sets and all components) to look at
histograms of th distributions. In the center and right panels, we overlay the theoretical
standard normal distribution that one would expect to see after standardizing the weights (by
data set and component). Using the usual mean and distribution (center panel), the actual weights
are slightly more conservative (i.e., concentrated near zero) than expected. Using the robust
median and MAD (right panel), the weights are less conservative (with heavier tails). This figure
suggests that we might want to use the usual method, and decide on significance purely from the
theoretical distribution rather than from deviations away from that distribution.
```{r fig24, fig.cap = .tag(24, "Histograms of all weights (combined)."), fig.width=12, fig.height=6}
opar <- par(mfrow = c(1, 3))
...
yy <- dnorm(xx)
lines(xx, yy, col = "red", lwd=2)
hist(robust, breaks = 123, main = "Robust Weights", prob = TRUE)
lines(xx, yy, col = "red", lwd=2)
par(opar)
rm(opar)
```

Finally, we create yet another image (**Figure 25**), counting the number of significant features
from each data set for each component. As just suggested, we use the standardized weights. We feel
that this result is more reasonable than any thing we got looking at the top 20 lists. The most
contributions come from the biggest omics data sets (mRNA, methylation, and miR) with fewer from
MAF, RPPA, and clinical binary. (There are none from the clinical continuous data set.)
```{r fig25, fig.cap = .tag(25, "Number of significant contributions by data set and component."), fig.width=6, fig.height=6}
top1p <- aggregate(brute, list(datasrc), function(X) sum(abs(X) > 2.5758)) # top 1 percent
rownames(top1p) <- top1p[, 1]
top1p <- as.matrix(top1p[, -1])
image(1:7, 1:24, top1p, ylab = "Components", xlab = "Data Sets")
mtext(levels(datasrc),side = 3, at = 1:7, line = 0)
```
