...
r <- c(rep(0, A), 0:A)[-A]
g <- -c(-A:0, rep(0, A))[-A - 1]
RG <- rgb(r, g, rep(0, 64), maxColorValue = A)
rm(A, r, g)
RG <- heat.colors(64)
```
...
heat(cbin, cexCol = 0.7, cexRow = 0.5, col = RG)
cbin01 <- pickSignificant(cbin, 0.01)
heat(cbin01, cexCol = 0.7, cexRow = 0.7, col = RG)
```
...
heat(ccont, cexCol = 0.8, cexRow = 1.2, col = RG)
```
...
heat(cmaf01, cexCol = 0.7, cexRow = 0.4, col = RG)
```
...
heat(meth01, cexCol = 0.7, cexRow = 0.3, col = RG)
```
...
heat(rppa01, cexCol = 0.7, cexRow = 0.5, col = RG)
```
...
heat(mrna01, cexCol = 0.5, cexRow = 0.5, col = RG)
```
...
heat(mir01, cexCol = 0.7, cexRow = 0.5, col = RG)
```
