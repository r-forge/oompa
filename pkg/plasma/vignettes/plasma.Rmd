...
```
Next, we load ample data derived from teh esophageal cancer (ESCA) cohort of
...
leaving them out is to be able to show that `plasma` can find useful components from any of the included datasets and manage to extend them to the full cohort.
...
```{r fig01, fig.cap = "Overview of te ESCA data sets."}
...
```{r fig02, fig.cap = "Overview of training data."}
trainD <- MO[, splitVec == 1]
summary(trainD)
plot(trainD, main = "Train")
```

```{r fig03, fig.cap = "Overview of training data."}
testD <- MO[, splitVec == 0]
summary(testD)
