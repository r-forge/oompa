...
data set in each retained component. Some data sts have very different contribution patterns than others.
For example, the "Clinical Binary" data set appears to have significant outliers making large
contributions in almost every component, the MAF and RPPA data sets also frequently (but not
always) include such outliers.
```{r fig10, fig.cap = .tag(10, "Distributions of weights by data set and component."), fig.width = 10, fig.height = 10}
library(beanplot)
library(Polychrome)
data(palette36)
foo <- computeDistances(palette36[3:36])
colist <- as.list(palette36[names(foo)[1:7]])
opar <- par(mfrow = c(3, 3), mai = c(0.2, 0.5, 0.5, 0.1))
...
```{r fig11, fig.cap = .tag(11, "Histogram of weights by component."), fig.width = 10, fig.height = 10}
...
```{r fig12, fig.cap = .tag(12, "Distributions of standardized weights by data set and component."), fig.width = 10, fig.height = 10}
...
We also include plots of the histograms of distributions by component (**Figure 13**). None of
these really looks quite normal; they almost have some odd shape and signs of skewness. But they
are still better looking than the raw weights.
```{r fig13, fig.cap = .tag(13, "Histogram of standardized weights by component."), fig.width = 10, fig.height = 10}
opar <- par(mfrow = c(3, 3), mai = c(0.2,0.2, 0.5, 0.2))
for (i in which(colnames(contra) %in% mainterms)) {
...
are shown in **Figure 14**. Using the raw weights, the vast majority of contributions come from
...
```{r fig14, fig.cap = .tag(14, "Number of contributers to top twenty lists."), fig.width=9, fig.height=6}
...
In **Figure 15**, we pool all the weights 9across all data sets and all components) to look at
...
```{r fig15, fig.cap = .tag(15, "Histograms of all weights (combined)."), fig.width=9, fig.height=6}
opar <- par(mfrow = c(1, 2))
...
Finally, we create yet another image (**Figure 16**), counting the number of significant features
...
```{r fig16, fig.cap = .tag(16, "Number of significant contributions by data set and component."), fig.width=6, fig.height=6}
...
These are the mutated genes (from the MAF data set) associated with the component "`MAF1`".
```{r map}
pickA[nick %in% map, ]
```
...
including a clear role in cilia, flagella, and microfibrils.

We then extracted all gene names from the MAF, mRNASeq, and RPPA data sets and used thm to perform a
gene enrichment (pathway) analysis using ToppGene []. Associated GeneOntology Biological Process
