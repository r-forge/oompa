...
in the top denrogram). Second, we hypothesize that some
...
those thatdo not have highly ranked weights (by absolute value) in any component. 
```{r shrink}
...
We can apply the same method to visualize contributors from each of the different omics data sets.
As a second illustration, we look at the standardized weights from the mRNA data set in the compoentns
