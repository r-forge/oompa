...
that are common to, and able to explain the data well, all the omics data sets under
...
# Methods
We must load the `plasma` package.
```{r pkgs}
...
algorithm (`plsRcox' Version 'r packageVersion(plsRcox)` [Bertrand F, Bastien P, Meyer N, Maumy-Bertrand M (2014). “plsRcox, Cox-Models in a high dimensional setting in R.” In Proceedings of User2014!, 177.]) to the time-to-event outcome
...
(`plsr' Version 'r packageVersion(pls)` [Mevik, B.-H., & Wehrens, R. (2007). The pls Package: Principal Component and Partial Least Squares Regression in R. Journal of Statistical Software, 18(2), 1–23. https://doi.org/10.18637/jss.v018.i02]) to learn how to predict the coefficients of
...
4. The second step in the algorithm creates a secondary layer of components. We do not give
   these components a separate name. They are not an itemn or particular focus; we view them
...
Our interest now turns to understanding how the features from individual omics data sets
contribute to the components that are used in the final model.
...
These arise from the secondary components that learn how to extend components from one omics
data set to the others. Those models use linear combinations of features as linear predictors
of the primary components. The `getAllWeights` function combines these two levels of linear
coefficients to compute the weight of each feature contributing to each primary component.
